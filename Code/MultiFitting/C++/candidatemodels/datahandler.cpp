...
/* ================================ FINAL LEAST SQUARES ============================= */
// Sets the upper and lower bounds for all model paratmers in the optimisation procedure.
// Note that each range is unique to each epidemic type.
void Handler::initial_param_bound(){
  ...
  betaIrsirUp = 0.0005;
  betaIrsirDown = 0.00002;
  gammaIrsirUp = 0.0005;
  gammaIrsirDown = 0.00002;
...
void Handler::realtime_fit_multi(double targetRSq){
  ...
  vector<vector<double> > combinedResults, residuals, combinedResultsTemp,saveResults, tomorrowData,ttData; // 
  vector<vector<vector<double> > > componentResults, componentResultsTemp;
  double RSquare, tempRSquare, SSE, tempSSE, median_data;
  int iterations, tempIterations, remove_index;;
  double bestAICc, tempAICc, RMSE, AICc;
  double tomorrowSSE, ttSSE,futureSSE;
  double tomorrowRSquare, ttRSquare,futureRSquare;
  double MAD;
  ofstream myfile; 
  ...
  for(unsigned int i = 7;i<this->current_data.size();++i){
    ...
    this->future_data.clear();
    // Store all data up to the current index and update epidemics with data
    for(unsigned int j = 0; j < i; ++j){
      this->temp_data.push_back(this->current_data[j]);
    } 
    for(unsigned int j = temp_data.size();j<current_data.size();++j){
      this->future_data.push_back(this->current_data[j]);
    }
    ...
    if(temp_data.size() < current_data.size()){
      tomorrowData.clear(); ttData.clear();
      tomorrowData = temp_data;
      tomorrowData.push_back(current_data[temp_data.size()]);
      ttData = temp_data;
      ttData.push_back(ttData[ttData.size()-1]);
      ttData[ttData.size()-1][0] = ttData[ttData.size()-2][0] + 1;
      tomorrowSSE = calculate_SSE(current_model,tomorrowData);
      ttSSE = calculate_SSE(ttData, tomorrowData);
      futureSSE = calculate_SSE(current_model,future_data);
      tomorrowRSquare = 1- tomorrowSSE/SStot(tomorrowData,1);
      ttRSquare = 1 - ttSSE/SStot(tomorrowData,1);
      futureRSquare = 1 - futureSSE/SStot(future_data,1);
      cout << "TT Rsq: " << ttRSquare << "; Future Rsq: " << futureRSquare << "; Tomorrow RSq: " << tomorrowRSquare << endl;
    }
      
    // Calculate MAD 
    median_data = calculate_median(temp_data);
    MAD = calculate_MAD(temp_data, median_data);
    ...
    // Store statistics in results vector
    printcon(finalParams);
    finalParams.push_back(RSquare);
    finalParams.push_back(SSE);
    RMSE = sqrt((SSE/temp_data.size()));
    finalParams.push_back(RMSE);
    finalParams.push_back(MAD);
    saveResults.push_back(finalParams);

    if(temp_data.size() < current_data.size()){
      finalParams.push_back(futureSSE);
      finalParams.push_back(futureRSquare);
      finalParams.push_back(tomorrowSSE);
      finalParams.push_back(tomorrowRSquare);
      finalParams.push_back(ttSSE);
      finalParams.push_back(ttRSquare);
    }
    ...
    /* Save current parameters and statistics */
    myfile.open ((saveLocation + "/results.csv"),std::ios_base::app); 
    for(unsigned int j = 0;j < finalParams.size();++j){
      myfile << finalParams[j] << ",";
    }
    myfile << "\n";
    myfile.close();
  }
...
vector<double> Handler::convert_all_params_back(vector<double> pars){
  ...
  for(unsigned int i = 0;i<this->epidemicSizes.size();++i){
    ...
    temporary = convert_parameters_back(temporary,this->epidemicSizes[i],this->epidemics[i]->return_seed_time(),this->epidemics[i]->return_min_time(),this->epidemics[i]->return_detection_time(), epidemics[i]->return_type());
...
vector<double> Handler::convert_parameters_back(vector<double> pars, int number, double detection, double lowerTime, double upperTime, EpiType _type){
  vector<double> temporary;
  int temp_numb;
  temp_numb = number;
  if(temp_numb == 3){
    ...
    if(useLogistic){
      ...
      temporary.push_back(logistic(pars[4],lowerTime,upperTime));
    }
    else {
      temporary.push_back(exp(pars[0]));
      temporary.push_back(exp(pars[1]));
...
vector<double> Handler::generate_seed_parameters(){
  ...
  for(unsigned int i = 0;i<epidemics.size();++i){
    ...
    tempor = convert_parameters_forward(tempor, epidemicSizes[i], epidemics[i]->return_seed_time(), epidemics[i]->return_detection_time(), epidemics[i]->return_min_time(), epidemics[i]->return_type());
...
vector<double> Handler::rand_params(EpiType _type){
  ...
  case spike:
    ...
    break;
  case irsir:
    beta = (rand()%25+8)/100000.0;
    gamma = (rand()%25+8)/100000.0;
    params.push_back((beta));
    params.push_back((gamma));
    params.push_back((s0));
    break;
  case serir:
    beta = (rand()%42+6)/1000.0;
    gamma = (rand()%140+10)/1000.0;
    alpha = (rand()%80+5)/1000.0;
    mu = (rand()%80+5)/1000.0;
    gammaSerirDown = beta;
    betaSerirUp = gamma;
    params.push_back((beta));
...
// Update detection/seed time of latest epidemic based on passed parameters. NOT USED
...
// NOTE USED - idea would be to prevent all epidemics from including T0 in optimisation
...
// NOT USED - idea would be to include T0 of latest epidemic in optimisation procedure (if all others
// deactivated)
...
// Saves the passed results and epidemic types into a .csv file
...
double Handler::calculate_SSE(vector<vector<double> > data1, vector<vector<double> > data2){
  ...
  while(i < data1.size() && j < data2.size()){
    ...
    if(abs(data1[i][0]- data2[j][0])<EPSILON){
      sse += pow((data1[i][1] - data2[j][1]),2.0);
      i++;
      j++;
    }
    //If first dataset is still before second, full difference
    else if(data1[i][0] < data2[j][0]){
      //sse += pow(data1[i][1],2.0);
      i++;
    }
    //...
    else{
...
// Calculate the median of a given set of data (second column)
double Handler::calculate_median(vector<vector<double> > data){
  vector<double> sortedValues;
  double temp;
  double median = 0.0;
  int size;
  // Get all values to be sorted
  for(unsigned int i = 0;i<data.size();++i){
    sortedValues.push_back(data[i][1]);
  }
...
// Calculates the MAD score for a given data set with a given median
double Handler::calculate_MAD(vector<vector<double> > data, double median){
  vector<vector<double> > temp = data;
  
  for(unsigned int i = 0;i<data.size();++i){
    temp[i][0] = i;
    temp[i][1] = abs(data[i][1] - median);
  }
  
  return(1.4826*calculate_median(temp));
}
...
// Transforms argument x into the logistic space. That is, from -infty -> +infty to xmin -> xmax
double Handler::logistic(double x, double xmin, double xmax){
  return(((xmax-xmin)/(1.0+exp(-x)))+xmin);
}

// Transforms argument x into the logistic space. That is, from xmin -> xmax to -infty to +infty
double Handler::logit(double y, double xmin, double xmax){
  return(-log(((xmax-xmin)/(y-xmin))-1.0));
}
    
// Calculates the AICc score
...
// Destructor. Handles the deletion of all epidemic instances
Handler::~Handler(){
...
void Handler::plotGraphMulti(vector<vector<vector<double> > > finalResults, vector<vector<double> > totalResults, vector<vector<double> > data, int index, vector<double> parameters, double _RSquare, int column){
  ...
  string colours[19] = {"blue","red","orange","cyan","violet","yellow","black","pink","green","gray","brown","blue","red","orange","cyan","violet","yellow","black","pink"};
  int j = 0;
