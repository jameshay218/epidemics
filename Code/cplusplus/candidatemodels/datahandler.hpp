...
private:
  ...
  vector<vector<double> > current_data, current_model, temp_model;
...
public:
  ...
  double optimiseEpidemics(vector<double> &parameters, vector<vector<double> > data, vector<vector<double> > &results, vector<vector<vector<double> > > &allResults);
